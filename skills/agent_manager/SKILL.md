...
- サブタスクを新しいエージェントに **委譲** したいとき（「〜をサブエージェントに任せて」「この作業は office エージェントに委譲して」）は、コアToolの `delegate_task` を使う（このスキルのツールではなく常時利用可能）
...
- 委譲（`delegate_task`）: いま実行中のまま新しいサブエージェントを起動し、完結したサブタスクを丸ごと任せて結果だけ受け取る。`task` に必要な情報を全部書く（サブエージェントは会話履歴を引き継がない）。特定プロファイルに任せたいときは `agent` にその名前を渡し、省略すればライブラリ全体（default）で走る。長い調査や独立作業を切り出すのに向く。用途特化エージェントを新規に作るのは create_agent、既存構成に一時的な作業を任せるのが delegate_task、と使い分ける。
